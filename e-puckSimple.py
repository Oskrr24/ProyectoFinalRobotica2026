...
SCENARIO = "simple"   # solo informativo (usado en nombre de log y prints)

# Cuántas celdas inflar alrededor de cada obstáculo.
# Con cell_size=0.1 m y robot ~5 cm de radio: 1 celda = margen seguro mínimo.
...
# Calibrado desde MundoSimple.wbt.
# Arena: RectangleArena floorSize 2 2 -> x,y en [-1.0, 1.0].
# Robot: yaw ~= 1.566 rad (~90° = mirando +y global).
# Meta visual: Rabbit.
CONFIG = {
    "cell_size_m": 0.1,
    "map_min_x": -1.0,
    "map_max_y": 1.0,
    "start_world": (-0.653213, -0.456901),

    "goal_world": (-0.16, 0.67),

    "initial_heading_rad": 1.56635,

    "start": (3, 14),
    "goal": (8, 2),
    # Grilla 20×20, cada celda = 0.1 m.
    # '#' = obstáculo, '.' = libre, 'S' = inicio, 'G' = meta.
...
def run_robot():
    ...
    cruise_speed      = 0.06    # m/s velocidad normal
    slow_speed        = 0.03    # m/s al girar mucho o acercarse
    k_angle           = 3.5     # ganancia proporcional angular
    max_omega         = 5.0     # rad/s límite de giro
    angle_deadband    = 0.08    # rad: zona muerta para reducir oscilación en recto
    ...
    SIDE_SOFT    = 90     # Corrección lateral suave
    SIDE_HARD    = 200    # Corrección lateral agresiva
    EMA_ALPHA    = 0.30   # Peso de la lectura actual en el filtro exponencial

    # --- Detección de bloqueo ---
    stuck_timeout     = 3.0    # segundos sin avance → maniobra escape
    stuck_dist_thresh = 0.015  # metros: si no avanzó esto en stuck_timeout, está bloqueado
    ...
    def write_log_row(
        t, target_x, target_y, dist_wp, angle_err,
        left_spd, right_spd, raw_values, filtered_values, state,
    ):
        ...
        if gps_compass_available:
            ...
            # Compass de Webots da el vector "norte" en el plano XY del robot;
            # el heading real se obtiene igual que en el e-puck estándar.
        ...
        log_writer.writerow([
            ...
            f"{dist_wp:.4f}", f"{angle_err:.4f}",
            f"{left_spd:.4f}", f"{right_spd:.4f}",
            *[f"{raw_values[i]:.1f}" for i in sensor_indices],
            *[f"{filtered_values[i]:.1f}" for i in sensor_indices],
    ...
    while robot.step(timestep) != -1:
        ...
        # ---- Lectura de sensores ----
        raw_ps = [s.getValue() for s in ps]
        for i, raw_value in enumerate(raw_ps):
            if filtered_ps[i] is None:
                filtered_ps[i] = raw_value
            else:
                filtered_ps[i] = (
                    EMA_ALPHA * raw_value
                    + (1.0 - EMA_ALPHA) * filtered_ps[i]
                )
        ps_val = filtered_ps
        ...
        # Emergencias con lectura cruda; correcciones suaves con lectura EMA.
        front_max_raw = max(raw_ps[7], raw_ps[0])
        front_max_filtered = max(front_left_val, front_right_val)
        ...
        if dist_wp < tol:
            if is_last:
                ...
                left_motor.setVelocity(0.0)
                right_motor.setVelocity(0.0)
                final_angle_err = normalize_angle(
                    math.atan2(dy_wp, dx_wp) - theta
                ) if dist_wp > 1e-9 else 0.0
                write_log_row(
                    t, target_x, target_y, dist_wp, final_angle_err,
                    0.0, 0.0, raw_ps, ps_val, "goal",
                )
        ...
        if t < escape_until:
            ...
            v_base = 0.0
            # La emergencia compara los sensores frontales crudos.
            turn_dir = -1.0 if raw_ps[7] > raw_ps[0] else 1.0
            omega_base = turn_dir * 4.0

        elif front_max_filtered > FRONT_SOFT:
            ...
            blend = min(
                (front_max_filtered - FRONT_SOFT) / (FRONT_HARD - FRONT_SOFT),
                1.0,
            )
        ...
        # ---- Detección de bloqueo ----
        if state in ("escape", "emergency_turn"):
            # Estas maniobras no forman parte de una ventana normal de avance.
            last_progress_time = t
            last_progress_dist = executed_distance
        else:
        ...
        if t - last_log_time >= 0.25:
            write_log_row(
                t, target_x, target_y, dist_wp, angle_err,
                left_spd, right_spd, raw_ps, ps_val, state,
            )
            log_file.flush()
            last_log_time = t
